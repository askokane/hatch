...
- **Project group chats are a second messaging stack, not a wider `Thread`.** Every project has one room and its members are the project's team. The obvious implementation was to relax `Thread.introRequestId` and let a thread hold N members, and it was rejected because the two-person assumption is not written down in one place: `otherMemberId()` asks the database for "the member who is not me", presence is a single `otherTyping`/`otherLastReadAt` pair, and a block closes the conversation — which is right when the conversation *is* the two of you and wrong on a team of six. Making each of those branch on a discriminator puts a conditional in the 3s poll and the unread count, where the wrong branch shows someone a room they are not in. So `ProjectChat`/`ProjectChatMessage` and `src/lib/project-chat-core.ts` sit alongside the thread stack, and `Thread` is untouched.
- **The group chat has no member table, on purpose.** Its membership *is* `Membership` — not a copy kept in agreement with it. A parallel list would have to be written by `inviteMemberAction`, `removeMemberAction` and `transferOwnershipAction` as well, and the failure mode of that copy drifting is a person reading a room they were removed from. So authorization is the same `assertProjectMember` the project pages use, and the two pieces of per-person chat state (`chatLastReadAt`, `chatTypingUntil`) are columns on the membership row. Removing someone from a project removes them from its chat in the same `DELETE`, with no second write to forget — which is what `e2e/18-project-group-chat.spec.ts` asserts by deleting the membership directly and then checking both the page and the endpoint.
- **A block hides one person; it does not close the room.** In a thread of two, blocking ends the conversation because there is nothing left of it. In a team room it would let one person silence five, so instead the blocker simply stops being sent the blocked member's messages and typing — in both directions, since someone who blocked you has decided they want nothing to do with you and a shared room should not quietly be the exception. Neither side is told anything; the messages are just not in the page, which is indistinguishable from that person not having written any. The filter is applied **inside** the transcript query (`blockedFrom` → `notIn`), never to its result: dropping rows from an already-limited page yields short pages and a "load earlier" control that walks backwards in uneven steps. `/api/nav-counts` carries the same exclusion as a `NOT EXISTS`, so the badge never counts a message the transcript will not show.
- **Group presence is plural.** `typingNames` drives "Maya and Ali are typing…" (named people stop at two — past that it is "and 3 others", because five names push the transcript up a line every time one of them pauses), and the receipt is a count: `✓✓ seen by 2 of 4`, against the team size. A per-member read list was deliberately not built — it would put every teammate's reading habits on every poll response to answer a question the sender was not asking. A closed project's chat is read-only, which is the only reason a group composer ever closes.
